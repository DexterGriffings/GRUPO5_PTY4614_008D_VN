...
def envio(request):
    if request.method == 'POST':
        ...
        subject = 'Seguimiento por bajo rendimiento en cuestionario sobre becas'
        message = f'Estimado/a {nombre}. \n\n Desde el equipo de BecaFacil notamos un bajo rendimiento en nuestro cuestionario, en el cual obtuvo un puntaje de {puntaje}. \n\n Dado que esta información es fundamental para orientar adecuadamente el proceso de postulación y beneficios, solicito amablemente contactar al área de Admisión de Duoc UC a la brevedad para analizar la situación y definir los pasos a seguir. \n\n Atentamente, \n\n Equipo BecaFacil'
...
def eliminar_alumno(request):
    if request.method == 'POST':
        ...
        try:
            registro = Cuestionario.objects.get(rut=rut)
            registro.delete()
            return JsonResponse({'status': 'success'})
        except Cuestionario.DoesNotExist:
            return JsonResponse({'status': 'fail', 'message': 'Registro no encontrado'})
